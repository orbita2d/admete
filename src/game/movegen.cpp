...
void generate_pseudolegal_moves(const Board &board, std::vector<Move> &moves) {
    for (Square::square_t i = 0; i < 64; i++) {
        ...
        if (piece.is_knight()) {
            get_knight_moves<colour>(board, i, moves);
        } else if (piece.is_pawn()) {
            get_pawn_moves<colour>(board, i, moves);
        } else if (piece.is_rook()) {
            get_rook_moves<colour>(board, i, moves);
        } else if (piece.is_bishop()) {
            get_bishop_moves<colour>(board, i, moves);
        } else if (piece.is_queen()) {
            get_queen_moves<colour>(board, i, moves);
        } else if (piece.is_king()) {
            get_king_moves<colour>(board, i, moves);
        }
    }
}
