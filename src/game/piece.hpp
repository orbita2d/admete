...
class Piece {
  public:
    ...
    constexpr Piece get_piece() const { return Piece(value & PMASK); }
    ...
    constexpr bool is_white() const { return (value & CMASK) == 0x08; }

    constexpr bool is_black() const { return (value & CMASK) == 0x10; }

    constexpr bool is_blank() const { return value == 0x00; }

    constexpr bool is_pawn() const { return (value & PMASK) == 0x01; }

    constexpr bool is_knight() const { return (value & PMASK) == 0x02; }

    constexpr bool is_bishop() const { return (value & PMASK) == 0x03; }

    constexpr bool is_rook() const { return (value & PMASK) == 0x04; }

    constexpr bool is_queen() const { return (value & PMASK) == 0x05; }

    constexpr bool is_king() const { return (value & PMASK) == 0x06; }

    constexpr bool is_piece(const PieceType p) const { return (value & PMASK) == p + 1; }
...
inline Colour to_enum_colour(const Piece p) { return p.is_white() ? WHITE : BLACK; }
inline PieceType to_enum_piece(const Piece p) { return PieceType((p.get_value() & PMASK) - 1); }
